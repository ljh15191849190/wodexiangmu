...
var statusDictionary_vue = new Vue({
    el: '#apps',
    data: {
        S_Status_Dictionary: {
            //对象类型
        ...
        if (keyValue) {
            ...
            PageCommon.GetFormData({
                ...
                , success: function (data) {
                    statusDictionary_vue.S_Status_Dictionary = data;
                }
            });
        }
    },
    methods: {
        ...
        rowEditCallback: function (cell, titleData, rowData) {
            ...
            else if (titleData.mData == "Object_Type") {
                //获取模板的html
                cell.node().innerHTML = $("#temp_Object_Type").html();
                $("#div_temp_Object_Type").attr("id", "div_temp_Object_Type_" + cell.index().row + "_" + cell.index().column);
                //初始化VUE模板
                var cheld = new Vue({
                    el: "#div_temp_Object_Type_" + cell.index().row + "_" + cell.index().column,
                    data: {
                        Object_Type: cell.data()
                    }
                });
                //设置控件在DataTables上
                var jqInputs = $('input', cell.node());
                $(jqInputs)[0].select();
                $(jqInputs).attr('id', "Object_Type_" + cell.index().row + "_" + cell.index().column);
                $(".outer", cell.node()).width(tdWidth);
            }
        ...
        Save: function () {
            ...
            //获取grid数据
            var data = PageCommon.GetDataTableData("StatusDictionaryList");
            statusDictionary_vue.S_Status_Dictionary.S_Status_Dictionary_List = [];
            //组织明细数据 过滤空行
            $.each(data, function (index, item) {
                if (item.Code != "" && item.Name != "" && item.Show_Order != "") {
                    var obj = {};
                    obj.Object_Type = statusDictionary_vue.S_Status_Dictionary.Object_Type;
                    obj.Code = item.Code;
                    ...
                    statusDictionary_vue.S_Status_Dictionary.S_Status_Dictionary_List.push(obj);
                }
            });
            //验证至少输入一条明细
            if (!statusDictionary_vue.S_Status_Dictionary.S_Status_Dictionary_List.length > 0) {
            ...
            PageCommon.SubmitForm({
                url: "Save",
                param: { "statusDictionaryGroup": statusDictionary_vue.S_Status_Dictionary, "keyValue": [keyValue] },
                success: function () {
                    //判断编辑还是新增
                    if (!!!keyValue) {
                        PageCommon.Reload();
