...
var InboundFormDetailCancel_vue = new Vue({
    el: '#apps',
    data: {
        ...
        //编辑表格id数组
        tableid: ['InboundFormCancelDetailList'],
        //其他原因不可编辑
        noedite: true,
    },
    mounted: function () {
        ...
        $('#InboundFormCancelDetailList').on('dblclick', 'tbody tr', function () {
            PageCommon.ModalOpen({
                ...
                height: "600px",
                btn: ["确定"],
                url: '/Purchase/PurchaseInboundFormCancel/StockEnquiries',
                callBack: function (iframeId) {
                    var list = PageCommon.ChildrenFrames(iframeId).PurchaseleadCancel_vue.Add();
                    ...
                    PageCommon.RefreshTablePage("InboundFormCancelDetailList");
                }
            })
        })
        //控件拖动
    ...
    methods: {
        ...
        Save: function () {
            var that = this;
            $('#InboundFormCancelDetailList').DataTable().cell.blur();
            //数据验证
            if (!PageCommon.CheckPageInput()) {
                return;
            }
            //获取定单明细表数据
            var detaildata = PageCommon.GetDataTableData("InboundFormCancelDetailList");
