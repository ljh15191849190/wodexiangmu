   var vmSaleInvoiceRedRushApply = new Vue({
        el: '#apps',
        data: {
            productrange: {
                first: "",
            ...
            childSearchModel: [
                  {
                      Field_Name: ["FA_Exception_Invoice_A_Detail.Exception_Invoice_Apply_Id"],
                      Parent_Key: ["Exception_Invoice_Apply_Id"],
                      Detail_Seach: 'DetailSeach',
                      Child_List: 'SaleInvoiceRedRushApplyDetailList',
                      Default_Child: "true"
                  }
            ],
            //表头查询条件
        ...
        methods: {
            ...
            Cancel: function () {
                var selectRow = PageCommon.GetTableSelectRow("SaleInvoiceRedRushApplyList");
                //画面选择一条数据
                ...
                if (selectRow[0].Invoice_Status == '06') {
                    PageCommon.ShowMessageArrayRight("E2072", new Array("已全部红冲"));
                    return;
                }
                if (selectRow[0].Approval_Status == '03') {
                    PageCommon.SubmitForm({
                        url: "SaleInvoiceRedRush",
                        param: { exceptionInvoiceApply: selectRow[0] },
                        success: function (data) {
                            PageCommon.RefreshTablePage("SaleInvoiceRedRushApplyList");
                        }
                    });
            ...
            Delete: function () {
                ...
                if (selectRow[0].Approval_Status == '01' || selectRow[0].Approval_Status == '04')
                {
                    ...
                    PageCommon.SubmitDetialForm({
                        ...
                        type: "0",
                        success: function () {
                            PageCommon.RefreshTablePage("SaleInvoiceRedRushApplyList");  //刷新数据
                        },
                        close: true
                    })
                } else {
            ...
            Export: function (obj) {
                PageCommon.ModalExportOpen({
                    tableName: 'FA_Exception_Invoice_Apply_RedRush',
                    menu_Id: 'M000007',
                    list_Id: 'SaleInvoiceRedRushApplyList',
                    systemKey: 'FinancialApplication',
                    vm: vmSaleInvoiceRedRushApply
                })
            },
            //设置
            Set: function () {
            ...
            //查询部门名称
            Search: function () {
                vmSaleInvoiceRedRushApply.searchModel = $("#searchForm").GetSearchModel({ Menu_Id: "M000010", List_Id: "SaleInvoiceRedRushApplyList" });
            },
            mClear: function () {
                this.Invoice_Date = '';
