 var vmSaleInvoiceInvaildApply = new Vue({
        el: '#apps',
        data: {
            productrange: {
                first: "",
            ...
            childSearchModel: [
                  {
                      ...
                      Child_List: 'SaleInvoiceInvalidApplyDetailList',
                      Default_Child: "true"
                  }
            ],
            //表头查询条件
        ...
        methods: {
            ...
            Cancel: function () {
                var selectRow = PageCommon.GetTableSelectRow("SaleInvoiceInvalidApplyList");
                //画面选择一条数据
                if (selectRow.length != 1) {
                    PageCommon.ShowMessageRight("E2008");
                    return;
                }
                if (selectRow[0].Invoice_Status == '03') {
                    PageCommon.ShowMessageArrayRight("E2072", new Array("已作废"));
                    return;
                }
                if (selectRow[0].Approval_Status == '03') {
                    PageCommon.SubmitForm({
                        url: "SaleInvoiceInvalid",
                        param: { exceptionInvoiceApply: selectRow[0] },
                        success: function (data) {
                            PageCommon.RefreshTablePage("SaleInvoiceInvalidApplyList");
                        }
                    });
            ...
            Delete: function () {
                ...
                PageCommon.SubmitDetialForm({
                    ...
                    type: "0",
                    success: function () {
                        PageCommon.RefreshTablePage("SaleInvoiceInvalidApplyList");  //刷新数据
                    },
                    close: true
                })
            },
            ...
            Export: function (obj) {
                PageCommon.ModalExportOpen({
                    tableName: 'FA_Exception_Invoice_Apply_Invalid',
                    menu_Id: 'M000005',
                    list_Id: 'SaleInvoiceInvalidApplyList',
                    systemKey: 'FinancialApplication',
                    vm: vmSaleInvoiceInvaildApply
                })
            },
            //设置
            Set: function () {
            ...
            //查询部门名称
            Search: function () {
                vmSaleInvoiceInvaildApply.searchModel = $("#searchForm").GetSearchModel({ Menu_Id: "M000005", List_Id: "SaleInvoiceInvalidApplyList" });
            },
            mClear: function () {
                this.Invoice_Date = '';
