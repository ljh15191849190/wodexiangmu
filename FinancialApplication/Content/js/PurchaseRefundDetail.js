...
var NewPurchaseRefundApply_vue = new Vue({
    el: '#apps',
    data: {
        acceptBill: {
        ...
        if (relevantnumber) {
            PageCommon.GetFormData({
                ...
                success: function (data) {
                    that.FA_Purchase_Refund_Apply = data;
                    //判断状态
    ...
    methods: {
        ...
        Save: function () {
            ...
            that.FA_Purchase_Refund_Apply.Refund_Amount = parseFloat(that.FA_Purchase_Refund_Apply.Goods_Pay) + parseFloat(that.FA_Purchase_Refund_Apply.Earnest) + parseFloat(that.FA_Purchase_Refund_Apply.Freight) + parseFloat(that.FA_Purchase_Refund_Apply.Other)
            PageCommon.SubmitForm({
                url: "Save"
                     , param: { purchaseRefundApply: that.FA_Purchase_Refund_Apply }
                     , success: function (data) {
                         //保存后根据返回值加载页面
                         var relevantnumber = data.Result_Model.Purchase_Refund_Apply_Id;
        ...
        Approval: function () {
            ...
            PageCommon.SubmitForm({
                ...
                param: { purchaseRefundApply: that.FA_Purchase_Refund_Apply },
                success: function (data) {
                    window.location.href = PageCommon.SetUrlParameter("Detail", "relevantnumber=" + that.FA_Purchase_Refund_Apply.Purchase_Refund_Apply_Id + ',' + '&sourcetype=0&isedit=1');//sourcetype：列表0、审核1、提醒2   isedit：0可修改 1不可修改
                }
            });
        },
        //审批意见
