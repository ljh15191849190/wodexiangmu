    ...
    var PriceApply_vue = new Vue({
        ...
        methods: {
            ...
            Save: function () {
                ...
                //设置统购外购
                PriceApply_vue.M_Price_Apply.Is_Purchase = "0";

                //联系人信息组织明细数据 过滤空行
                $.each(data, function (index, item) {
                    if (item.Product_Id != "" && item.Cost_Source != "") {
                        var obj = {};
                        obj.Is_Purchase = PriceApply_vue.M_Price_Apply.Is_Purchase,
                        obj.Price_Apply_Id = item.Price_Apply_Id;
                        obj.Detail_Number = item.Detail_Number;
                        obj.Product_Id = item.Product_Id;
                        obj.Product_Name = item.Product_Name;
                        obj.User_Enqueriy_Product_Name = item.User_Enqueriy_Product_Name;
                        obj.Quote_Product_Name = item.Quote_Product_Name;
                        ...
                        obj.Quote_Usefel_Life = item.Quote_Usefel_Life;
                        obj.Customer_Receive_Price = item.Customer_Receive_Price;
                        obj.Apply_Count = item.Apply_Count;
                        obj.Price = item.Price;
                        obj.Tax = item.Tax;
                        obj.Price_Tax_Sum = item.Price_Tax_Sum;

                        obj.Is_include_Tax = item.Is_Include_Tax_Name == '否' ? "1" : "0";
                        obj.Is_Include_Tax_Name = item.Is_Include_Tax_Name;
                        obj.Is_Commission = item.Is_Commission;
                        obj.Ty_Way = item.Ty_Way;
                        obj.Commission = item.Commission;
                        obj.Commission_Rate = item.Commission_Rate;
                        obj.Net_Price = item.Net_Price;
                        obj.Delivery_Date = item.Delivery_Date;
                        obj.Consult_Price_Answer_Id = item.Consult_Price_Answer_Id;
                        obj.Consult_Price_Answer_Detail_Number = item.Consult_Price_Answer_Detail_Number;
                        obj.Cost_Source = item.Cost_Source;
                        obj.Goods_Apply = item.Goods_Apply;
                        ...
                        obj.CY_Profit_Rate_Period = item.CY_Profit_Rate_Period;
                ...
                PageCommon.SubmitForm({
                 ...
                 , success: function (data) {
                     ...
                     window.location.href = PageCommon.SetUrlParameter("Detail", "relevantnumber=" + data.Result_Model.Request_Object.Price_Apply_Id + "," + data.Result_Model.Request_Object.Customer_Id + '&save=1');
                 }
                });
            },
            //编辑
