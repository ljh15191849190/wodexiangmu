...
var Contacter_vue = new Vue({
    ...
    data: {
        ...
        searchModel: {
            Where_Parameter_List:[{
                Field_Name: "C_Customer_Contacter.Customer_Id",
                Query_Type: "01",
                Expression: "02",
                Condition_Value1: "-1",
    ...
    methods: {
        ...
        Save: function () {
            ...
            $('#ContacterListEdit').DataTable().cell.blur();
            ...
            //获取grid数据
            var data = PageCommon.GetDataTableData("ContacterListEdit");
            Contacter_vue.C_Customer.C_Contacts_List = [];
            //组织明细数据 过滤空行
            $.each(data, function (index, item) {
                if (item.Name != "") {
                    ...
                    if (item.Telephone != "" || item.Mobile_Phone != "") {
                        ...
                        Contacter_vue.C_Customer.C_Contacts_List.push(obj);
                    }
                }
            });

            //验证至少输入一条明细
            if (!Contacter_vue.C_Customer.C_Contacts_List.length > 0) {
